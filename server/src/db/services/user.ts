...
export const findByNameEmail = async (user: Partial<User>, includePassword = false, limit = 10) => {
    try {
        const users = await UserModel.find({ $or: [{ name: user.name }, { email: user.email }] })
            .limit(limit)
            .lean();
...
export const update = async (id: string, updatedUser: Partial<User> & { password?: string }) => {
    if (!id) {
        return null;
    }
